...
colnames(basematrix) <- colnames(prosal[,10:14])
rownames(basematrix) <- names(summary(prosal$Plot))
...
for (i in 1:9) {   for (k in 1:5) {
  cwm[i,k] <- mean(subset(prosal[,k+9],prosal$Plot==names(summary(prosal$Plot))[i]))
  cwv[i,k] <- var(subset(prosal[,k+9],prosal$Plot==names(summary(prosal$Plot))[i]))
  cwr[i,k] <- max(subset(prosal[,k+9],prosal$Plot==names(summary(prosal$Plot))[i]))-min(subset(prosal[,k+9],prosal$Plot==names(summary(prosal$Plot))[i]))
  cws[i,k] <- skewness(subset(prosal[,k+9],prosal$Plot==names(summary(prosal$Plot))[i]))
  cwk[i,k] <- kurtosis(subset(prosal[,k+9],prosal$Plot==names(summary(prosal$Plot))[i]))
}}
...
plot <- names(summary(prosal$Plot))
traitsdata <- cbind(transect,plot,cwm,cwr,cwv,cws,cwk)
sal1 <- ggplot(prosal[,c(6,7,10)], aes(x=body, fill=Plot)) +  geom_density(aes(colour=Transect), alpha = 0, size=0.5)+theme(legend.position='none')
sal2 <- ggplot(prosal[,c(6,7,11)], aes(x=leg, fill=Plot)) +  geom_density(aes(colour=Transect), alpha = 0, size=0.5)+theme(legend.position='none')
sal3 <- ggplot(prosal[,c(6,7,12)], aes(x=eye, fill=Plot)) +  geom_density(aes(colour=Transect), alpha = 0, size=0.5)+theme(legend.position='none')
sal4 <- ggplot(prosal[,c(6,7,13)], aes(x=chel, fill=Plot)) +  geom_density(aes(colour=Transect), alpha = 0, size=0.5)+theme(legend.position='none')
sal5 <- ggplot(prosal[,c(6,7,14)], aes(x=fang, fill=Plot)) +  geom_density(aes(colour=Transect), alpha = 0, size=0.5)+theme(legend.position='none')
...
grid.arrange(sal1,sal2,sal3,sal4,sal5,cwm1,cwm2,cwm3,cwm4,cwm5,cwv1,cwv2,cwv3,cwv4,cwv5,cwr1,cwr2,cwr3,cwr4,cwr5,cws1,cws2,cws3,cws4,cws5,cwk1,cwk2,cwk3,cwk4,cwk5, ncol=5)
```
...
colnames(basematrix) <- colnames(prosal[,10:15])
rownames(basematrix) <- names(summary(prosal$Plot))
basematrix <- as.data.frame(basematrix)
traitssamp <- list()
...
for (j in nrep) {
  ...
  prosalsamp$Plot <- sample(prosalsamp$Plot, length(prosalsamp$Plot), replace = F)
  
  for (i in 1:9) {   
    for (k in 1:6) {
      cwm[i,k] <- mean(subset(prosalsamp[,k+9],prosalsamp$Plot==names(summary(prosalsamp$Plot))[i]))
      cwv[i,k] <- var(subset(prosalsamp[,k+9],prosalsamp$Plot==names(summary(prosalsamp$Plot))[i]))
      cwr[i,k] <- max(subset(prosalsamp[,k+9],prosalsamp$Plot==names(summary(prosalsamp$Plot))[i]))-min(subset(prosalsamp[,k+9],prosalsamp$Plot==names(summary(prosalsamp$Plot))[i]))
      cws[i,k] <- skewness(subset(prosalsamp[,k+9],prosalsamp$Plot==names(summary(prosalsamp$Plot))[i]))
      cwk[i,k] <- kurtosis(subset(prosalsamp[,k+9],prosalsamp$Plot==names(summary(prosalsamp$Plot))[i]))
    }
  }
